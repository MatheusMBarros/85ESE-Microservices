...
if (AUTH_SERVICE_URL) {
    app.use('/api/auth', createProxyMiddleware({
        ...
        onProxyReq: (proxyReq, req, res) => {
            console.log(`[Gateway] Redirecionando para Auth Service: ${req.method} ${req.path}`);
        }
    }));
}
...
if (USER_SERVICE_URL) {
    app.use('/api/users', authMiddleware, createProxyMiddleware({
        ...
        onProxyReq: (proxyReq, req, res) => {
            console.log(`[Gateway] Redirecionando para User Service: ${req.method} ${req.path}`);
        }
    }));
}
...
if (AGENDA_SERVICE_URL) {
    app.use('/api/agendas', authMiddleware, createProxyMiddleware({
        ...
        onProxyReq: (proxyReq, req, res) => {
            console.log(`[Gateway] Redirecionando para Agenda Service: ${req.method} ${req.path}`);
        }
    }));
}
...
app.listen(PORT, () => {
    ...
    if(!AUTH_SERVICE_URL || !USER_SERVICE_URL || !AGENDA_SERVICE_URL) {
        console.warn('AVISO: Uma ou mais URLs de serviço não estão definidas nas variáveis de ambiente. O roteamento pode não funcionar corretamente.');
    } else {
